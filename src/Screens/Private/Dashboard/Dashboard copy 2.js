...
const Dashboard = ({ navigation }) => {
  ...
  const getaAtiveStaff = () => {
    GET_WITH_TOKEN(
      ...
      success => {
        ...
        // Sync status state with API response
        if (success?.data?.active_staff) {
          const updatedStatus = {};
          success.data.active_staff.forEach(staff => {
            if (staff?.attendance_details?.status) {
              updatedStatus[staff.id] = staff.attendance_details.status;
            }
          });
          // Merge with existing status state (preserve pending updates)
  ...
  const handleStatusChange = (data, newStatus, modalData = null) => {
    // Update local status state immediately for UI feedback
    ...
    // Use modalData if provided (from modal), otherwise use leaveModal
    const leaveType = modalData?.leaveType || leaveModal.leaveType;
    const remarks = modalData?.remarks || leaveModal?.remarks;
    
    if (newStatus == 'absent') {
      if (leaveType?.value) {
        formdata?.append('leave_id', leaveType.value);
      }
    } else if (newStatus == 'late') {
      if (leaveType?.value) {
        formdata?.append('late_minutes', leaveType.value);
      }
    }
    ...
    POST_FORM_DATA(
      ...
      formdata,
      success => {
        // Keep the status in state even after refresh
        setStatus(prev => ({ ...prev, [data?.id]: newStatus }));
        getaAtiveStaff();
        SimpleToast.show(
          `Staff status is now ${newStatus}`,
          SimpleToast.SHORT,
        );
      },
      error => {
        ...
        SimpleToast.show('Failed to update status', SimpleToast.SHORT);
      },
      fail => {
        // Revert status on network failure
        setStatus(prev => {
          const updated = { ...prev };
          delete updated[data?.id];
          return updated;
  ...
  // Helper function to get current status for a staff member
  const getStaffStatus = staffItem => {
    // Check local state first (for immediate UI updates)
    if (status[staffItem.id]) {
      return status[staffItem.id];
    }
    // Fallback to API data
    return staffItem?.attendance_details?.status || null;
  };
      ...
      <View style={styles.card}>
        ...
        <View style={styles.statusRow}>
          <TouchableOpacity
            style={[
              styles.statusBtn,
              currentStatus === 'present' && styles.presentBtn,
            ]}
            onPress={() => handleStatusChange(item, 'present')}
          >
          <Image
            source={ImageConstant?.present}
            tintColor={currentStatus === 'present' ? '#fff' : '#000'}
            style={{ width: 16, height: 16, marginRight: 10 }}
          />
          <Typography
            type={Font?.Poppins_Medium}
            color={currentStatus === 'present' ? '#fff' : '#000'}
          >
        ...
        <TouchableOpacity
          style={[
            styles.statusBtn,
            currentStatus === 'absent' && styles.absentBtn,
          ]}
          onPress={() => {
            setLeaveModal({
              visible: true,
        ...
        >
          <Image
            tintColor={currentStatus === 'absent' ? '#fff' : '#000'}
            source={ImageConstant?.absent}
            style={{ width: 16, height: 16, marginRight: 10 }}
          />
          <Typography
            type={Font?.Poppins_Medium}
            color={currentStatus === 'absent' ? '#fff' : '#000'}
          >
        ...
        <TouchableOpacity
          style={[
            styles.statusBtn,
            currentStatus === 'late' && styles.lateBtn,
          ]}
          onPress={() => {
            setLeaveModal({
              visible: true,
        ...
        >
          <Image
            ...
            tintColor={currentStatus === 'late' ? '#fff' : '#000'}
          />
          <Typography
            type={Font?.Poppins_Medium}
            color={currentStatus === 'late' ? '#fff' : '#000'}
          >
        ...
        <View style={styles.modalBox}>
          ...
          <Button
            ...
            onPress={() => {
              ...
              } else if (leaveModal.type == 'late') {
                errors.leaveType = validators.checkRequire(
                  LocalizedStrings.Dashboard_model.late_Short || 'Late Duration',
                  leaveModal.leaveType?.value,
                );
              }
              ...
              if (isValidForm(errors)) {
                ...
                const modalData = {
                  ...
                  remarks: leaveModal.remarks,
                };
                const staffData = leaveModal.staff;
                const statusType = leaveModal.type;
                
                setLeaveModal({ ...leaveModal, visible: false });
                setModalErrors({});
                
                // Pass modal data to handleStatusChange
                handleStatusChange(staffData, statusType, modalData);
              }
            }}
